                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPTCX ought to be enough for anybody." - Bill Gates, 2017
    ui->zPTCXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPTCXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPTCX->setEnabled(false);
        ui->pushButtonMintzPTCX->setToolTip(tr("zPTCX is currently disabled due to maintenance."));

        ui->pushButtonSpendzPTCX->setEnabled(false);
        ui->pushButtonSpendzPTCX->setToolTip(tr("zPTCX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPTCXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPTCX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPTCX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPTCX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPTCX in ") +
...
void PrivacyDialog::on_pushButtonSpendzPTCX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPTCX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPTCX
        sendzPTCX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPTCX
    sendzPTCX();
}

void PrivacyDialog::on_pushButtonZPtcxControl_clicked()
{
    ZPtcxControlDialog* zPtcxControl = new ZPtcxControlDialog(this);
    zPtcxControl->setModel(walletModel);
    zPtcxControl->exec();
}

void PrivacyDialog::setZPtcxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzPtcxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPTCX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid PeerTubeCrypto Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPTCXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPTCX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PTCX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPTCX</b>";
    ...
    // use mints from zPtcx selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZPtcxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZPtcxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zptcx selector in case it was used
    ZPtcxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zPtcx Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPTCX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPTCX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPTCX "));
    ui->labelzPTCXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzPTCXSyncStatus->setVisible(fShow);
}
