...
#include "ui_zptcxcontroldialog.h"
...
std::list<std::string> ZPtcxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZPtcxControlDialog::listMints;

ZPtcxControlDialog::ZPtcxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZPtcxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZPtcxControlDialog::~ZPtcxControlDialog()
{
    delete ui;
}

void ZPtcxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPtcxControlDialog::updateList()
{
...
void ZPtcxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPtcxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZPtcx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPtcxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZPtcxControlDialog::GetSelectedMints()
{
...
void ZPtcxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
